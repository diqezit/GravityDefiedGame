...
public sealed class BikeDynamics(BikePhysics p)
{
    ...
    public void RebuildSprings()
    {
        ...
        (p.Spr[0], p.Spr[1]) = (
            new(BFrame, BFw, P.WheelBase, k, d),
            new(BFrame, BRw, P.WheelBase, k, d));
...
{
    enum CollCode { None, Resolve, DeepPen, HeadCrash }

    readonly record struct CollResult(CollCode Code, int Idx, float Nx, float Ny, float Pen);
    ...
    static readonly WheelFriction WF = WheelFriction.Default;
    static readonly BodyCfg BodC = BodyCfg.Default;
    static readonly CollResult NoHit = new(CollCode.None, -1, 0f, 0f, 0f);

    readonly PtState[] _curBackup = new PtState[BodyCount];
    ...
    bool StepSlice(float sliceDt, out CollResult cr)
    {
        ...
        var snap = (p.EngineTorque, p.FwGnd, p.RwGnd);
        ...
        (p.EngineTorque, p.FwGnd, p.RwGnd) = snap;
        return false;
    }

    public bool StepCollision(float dt)
    {
#if DEBUG
        p.ResetContactStats();
#endif
        ...
        return d2 >= P.MinAxleDist2 && d2 <= P.MaxAxleDist2;
    }
    ...
    CollResult ChkColl()
    {
        ...
        for (int i = 0; i < BodyCount; i++)
        {
            if (p.Crashed ? i == BHead : i is BUF or BUR)
                continue;
            ...
            if (i == BHead && !p.Crashed && code != CollCode.None)
            {
                if (Abs(vel.X * nx + vel.Y * ny) <= headVnLimit)
                    continue;
                return new(CollCode.HeadCrash, BHead, nx, ny, 0f);
            }

            switch (code)
            {
                case CollCode.DeepPen:
                    return new(CollCode.DeepPen, i, nx, ny, 0f);
    ...
    {
        float r = Max(p.Bods[idx].R, WF.MinWheelR);
        float rd = WF.RotDamp, td = WF.TangentDamp;
        float bounce = idx == BFw ? WF.BounceFront : WF.BounceRear;
        bool lean = p.LeanL || p.LeanR;
        ...
        pt.AngVel = rd * pt.AngVel - WF.SlipFriction * vt / r;

        if (Abs(pt.AngVel) < WF.StopThreshold)
        {
            pt.AngVel = lean ? -vt / r : 0f;

            if (!lean && p.FwGnd && p.RwGnd && Abs(vt) <= WF.StopThreshold * r)
                return (0f, 0f);
        }

        return (bounce, td * vt - WF.RollCouple * pt.AngVel * r);
    }
}
